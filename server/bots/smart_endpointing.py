"""REFERENCE ONLY — the original dual-LLM smart-endpointing design (pipecat 0.0.x era).

NOT wired into any pipeline and NOT importable on pipecat 1.5.0: it depends on 0.x
APIs that no longer exist (``OpenAILLMContextFrame``, ``LLMResponseAggregator``,
``google.ai.generativelanguage``). Kept as the design reference for the "sidecar
judge/extractor LLM" pattern — a dedicated small LLM with its own saturated prompt
whose structured verdict code acts on (see docs/PIPECAT_PATTERNS.md). The 1.5.0
replacement in production is ``FilterIncompleteUserTurnStrategies`` with per-node
rules (bots/base_bot.py + bots/payment.py).

Restored verbatim from commit c863efa; original wiring lived in that commit's
server/bots/base_bot.py (see git show c863efa:server/bots/base_bot.py).
"""
